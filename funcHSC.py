...
while True:
    ...
    # --- Import Data & Average ---
    read_tempSensor=(pd.read_csv(file_tempSensor)).set_index('DateTime ()') #read csv with pandas and replace index with date
    ...
    if overrideOff: #change controls only if override is OFF (0)
        new_valveCmd.to_csv(file_tempSensor,mode='w',header=True,index=True)
