...
def get_data(fileDay):
    ...
    if not errorActive:
        ...
        # Column to extract:
        columns = ['TA_M (C)', 'TF_M (C)', 'TA_M_TG (C)', 'TF_M_TG (C)', 'V2M', 'TA_U (C)', 'TF_U (C)', 'TA_U_TG (C)', 'TF_U_TG (C)', 'V1U','TA_G (C)', 'TF_G (C)', 'TA_G_TG (C)', 'TF_G_TG (C)', 'V3G','TA_OUT (C)','TW_IN (C)','TW_OUT (C)','TW_dT (C)','TW_dTon (C)','TP_CPU']
        dataset = []
